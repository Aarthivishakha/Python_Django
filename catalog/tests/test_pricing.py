...
from catalog.pricing import OrderQuote, QuoteCache, calculate_order_total, classify_order_size
...
class QuoteCacheTests(TestCase):
    def test_default_type_param_stores_and_retrieves(self):
        cache = QuoteCache()
        self.assertIsNone(cache.get("missing"))
        quote = OrderQuote(total_cents=900, size_tier="small")
        cache.set("k1", quote)
        self.assertIs(cache.get("k1"), quote)
