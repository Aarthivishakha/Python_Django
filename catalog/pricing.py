...
Uses Python 3.10 syntax: PEP 604 union types (X | None) without a
`from __future__ import annotations` shim, and a `match` statement (PEP
634) for the order-size classification.
"""
...
def classify_order_size(quantity: int) -> str:
    ...
    Written as a `match` statement (PEP 634) rather than if/elif to
    exercise Python 3.10's structural pattern matching.
    """
    match quantity:
        case q if q <= 0:
            return "invalid"
