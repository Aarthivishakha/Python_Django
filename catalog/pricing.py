...
Uses Python 3.8 syntax: the walrus operator (PEP 572), positional-only
parameters (PEP 570), and the f-string `=` debug specifier (all new
exactly in 3.8).
"""
...
def calculate_order_total(quantity: int, unit_price_cents: int, /, is_member: bool,
    ...
    `quantity` and `unit_price_cents` are positional-only (the `/`
    marker, PEP 570) since callers should never need to name them.
    ...
    if unit_price_cents < 0:
        raise ValueError(f'{unit_price_cents=} must be non-negative')
    ...
    # Walrus operator (PEP 572): compute and check the discount amount
    # in the same expression.
    if (discount := (subtotal * discount_pct) // 100) > subtotal:
        discount = subtotal
...
def classify_order_size(quantity: int) -> str:
    ...
    Plain if/elif - Python 3.8 has no match statement (added in 3.10).
    """
    if quantity <= 0:
        return 'invalid'
    elif quantity < 5:
...
class OrderQuote:
    ...
    def from_request(cls, quantity: int, unit_price_cents: int, is_member: bool,
        ...
        total = calculate_order_total(
            quantity, unit_price_cents, is_member=is_member, has_coupon=has_coupon,
            is_bulk_eligible=is_bulk_eligible, coupon_code=coupon_code)
