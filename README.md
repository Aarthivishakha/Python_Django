...
Python/Django project repository — `PYTHON_3.14` branch.
...
assistant's training cutoff, so no 3.14-specific language feature is
claimed here with confidence - if 3.14 adds something new (e.g. PEP 750
template strings were proposed for 3.14), it isn't reflected in this
code and would need verifying against real release notes.
...
- `quality/<tool>/` — all 14 tools from
  ...
  (not just the 6 that repo's own Python_3.14 folder lists): `beniget`,
