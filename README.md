...
Python/Django project repository — `PYTHON_3.15` branch.
...
assistant's training cutoff, so no 3.14/3.15-specific language feature
is claimed here with confidence - any such addition isn't reflected in
this code and would need verifying against real release notes.
...
- `quality/<tool>/` — all 14 tools from
  ...
  (not just the 1 - `crosshair` - that repo's own Python_3.15 folder lists): `beniget`,
