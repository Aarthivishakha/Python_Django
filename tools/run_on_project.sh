#!/usr/bin/env bash
# Runs the subset of tools/ that can meaningfully analyze source code
# against the real Django app (core/), not just each tool's own bundled
...
echo
echo "=== bandit (security) on core/ ==="
bandit -r "$ROOT/core" || true

echo
echo "=== beniget (def-use / dead defs) on core/ ==="
python "$ROOT/tools/beniget/run_on_core.py"
